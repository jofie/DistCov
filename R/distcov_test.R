...
distcov.test <- function(X,Y,test="permutation",b=499L,seed=NULL,affine=FALSE,bias_corr = TRUE,type.X="sample",type.Y="sample",metr.X="euclidean",metr.Y="euclidean",bandwidth=1)
{
    ...
    if (test=="permutation") {
        ...
        if (bias_corr == TRUE && type.X == "sample" && type.Y == "sample" &&
            ...
            dcov2 <- aijbij / n / (n - 3) - 2 * Sab / n / (n - 2) / (n - 3) + adotdot * bdotdot / n / (n - 1) / (n - 2) / (n - 3)
            ...
            reps <- sapply(1:b, function(t) {
             ...
             bidot <- Ydot + (2 * alphaY - n) * Y_samp - 2 * betaY
             Sab <- sum(aidot * bidot)
             gamma_1  <- PartialSum2D(X, Y_samp, rep(1, n))
             gamma_X  <- PartialSum2D(X, Y_samp, X)
             gamma_Y  <- PartialSum2D(X, Y_samp, Y_samp)
             gamma_XY <- PartialSum2D(X, Y_samp, X * Y_samp)
             aijbij <- sum(X * Y_samp * gamma_1 + gamma_XY - X * gamma_Y - Y_samp * gamma_X)
             res2 <- aijbij / n / (n - 3) - 2 * Sab / n / (n - 2) / (n - 3) + adotdot * bdotdot / n / (n - 1) / (n - 2) / (n - 3)
    ...
    if (test=="gamma") {
     ...
     distvarX <- distvar.meanoutput(X, affine, bias_corr=TRUE, type.X, metr.X, bandwidth)
     distvarY <- distvar.meanoutput(Y, affine, bias_corr=TRUE, type.Y, metr.Y, bandwidth)
