...
//   int n = X.size();
//   double Zdot = 0;
//   NumericVector Y_sort(n), Z_sort(n), sX(n + 1), sY(n + 1), gamma;
//   NumericVector IX(n), IY(n);
//   
//   for (int i = 0; i < n; i++) {
//     IX[IX0[i]] = i;
//     IY[IY0[i]] = i;
...
//     sY[i + 1] = sY[IY0[i]] + Z_sort[IY0[i]];
//     Zdot += Z_sort[i];
//   }
//   NumericVector gamma1(n);
//   gamma1 = DyadUpdate(IY, Z_sort);
//   for (int i = 0; i < n; i++) {
//     gamma[i] = Zdot - Z_sort[IX[i]] - 2 * sY[IY[IX[i]]] - 2 * sX[IX[i]] + 4 * gamma1[IX[i]];
//   }
//   return gamma;
// }
